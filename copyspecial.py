...
def get_special_paths(dirname):
    ...
    # creates an empty list "path_list" to eventually list all of the absolute
    # paths
    path_list = []
    ...
    for filename in paths:
        # regex statement checks to see whether or not the characters "__",
        # followed by a word of one or more letters, then another "__", exist
        # in the file name
        special_file = re.findall(r'__(\w+)__', filename)
        # If the regex statement's conditions are met, then the absolute path
...
def zip_to(path_list, dest_zip):
    """Returns a zip file containing the directory copy created
...
def main(args):
    ...
    # ns.from_dir gets the files from the given directory
    # calls the "copy_to" function if the "--todir" command is typed
    # typed, or calls the "zip_to" function if the "--tozip" command is
    # typed, otherwise it just prints the various paths listed in "path_list"
    if ns.todir:
